...
Biquad.LP = function biquadLP(cutoff, sampleRate, Q) {
  let K = Math.tan(Math.PI * cutoff / sampleRate)
...
Biquad.HP = function biquadHP(cutoff, sampleRate, Q) {
  let K = Math.tan(Math.PI * cutoff / sampleRate)
...
Biquad.BP = function biquadBP(cutoff, sampleRate, Q) {
  let K = Math.tan(Math.PI * cutoff / sampleRate)
...
Biquad.Notch = function biquadNotch(cutoff, sampleRate, Q) {
  let K = Math.tan(Math.PI * cutoff / sampleRate)
...
Biquad.Peak = function biquadPeak(cutoff, sampleRate, Q, peakGain) {
...
Biquad.LowShelf = function biquadLowShelf(cutoff, sampleRate, peakGain) {
  let V = Math.pow(10, Math.abs(peakGain) / 20)
  let K = Math.tan(Math.PI * cutoff / sampleRate)
...
Biquad.HighShelf = function biquadHighShelf(cutoff, sampleRate, peakGain) {
  let V = Math.pow(10, Math.abs(peakGain) / 20)
  let K = Math.tan(Math.PI * cutoff / sampleRate)
...
import * as myself from './Biquad.js'
